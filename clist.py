        ...
        try:
            ...
            if not ts:
                ...
                if not isinstance(ts, datetime):
                    ts = parse_date(ts)
                do_update = abs(datetime.utcnow() - ts).total_seconds() > int(since)
...
def create_poll(question, owner, message_id, locked=False):
    with DBContextManager() as conn:
        ...
        cursor = conn.cursor()
        sql = (
            "INSERT INTO polls (question, owner, message_id, locked) "
...
def create_pollopt(poll_id, emoji, meaning, owner, add_vote=False):
    with DBContextManager() as conn:
        ...
        cursor = conn.cursor()
        sql = (
            "INSERT INTO pollopts (poll, emoji, meaning, owner, votes) "
...
def get_or_create_deathsight(
    db_connection, killer, corpse, external_id, counts_for_kdr=False
):
    cursor = db_connection.cursor()
    cursor.execute(
        fmt_sql(
            "SELECT d.killer, d.corpse FROM deaths d " "WHERE d.external_id = %s;", 1
        ),
        (external_id,),
    )
    try:
        ...
        cursor.execute(
            fmt_sql(
                "INSERT INTO deaths (killer, corpse, external_id, kdr_count) "
                "VALUES (%s, %s, %s, %s)",
                4,
            ),
...
def get_or_create_toon(db_connection, name, api_url=None):
    ...
    sql = fmt_sql(
        (
            "SELECT c.city, c.level FROM characters c "
            "WHERE c.name = %s ORDER BY c.id DESC;"
        ),
        1,
    )
    ...
    try:
        ...
        cursor.execute(
            fmt_sql(
                ...
                % (", ".join(API_FIELDS), ", ".join("%s" for field in API_FIELDS)),
                len(API_FIELDS),
            ),
            [data.get(field, "unknown") for field in API_FIELDS],
        )
    return data
...
def get_poll_report(poll_id, message):
    with DBContextManager() as conn:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT message_id, question, owner FROM polls WHERE id = %s", (poll_id,)
        )
        ...
        report_msg = [
            "Poll %i posted by <@%s>" % (poll_id, owner),
            "> %s" % question.title().replace("'S", "'s"),
            "",
        ]
        sql = (
            "SELECT emoji, meaning, votes FROM pollopts "
...
def list_toons(update=False, quick=False, min_level=1, positive_kdr=None, api_url=None):
    if api_url is None:
        api_url = API_URL
    ...
    def is_kdr_positive(conn, name, positive_kdr=None):
        if positive_kdr is None:
            ...
            return True
        cursor = conn.cursor()
        cursor.execute("SELECT kdr FROM kdr WHERE killer=%s LIMIT 1", (name,))
        try:
    ...
    with DBContextManager() as conn:
        ...
        for toon in toons:
            ...
            if int(data["level"]) >= min_level and is_kdr_positive(
                conn, toon["name"], positive_kdr
            ):
...
def show_death_history(corpse=None, killer=None):
    with DBContextManager() as conn:
        ...
        if corpse:
            cursor.execute(
                fmt_sql("SELECT MIN(timestamp) FROM deaths " "WHERE corpse = %s;", 1),
                (corpse,),
            )
            try:
            ...
            cursor.execute(
                fmt_sql(
                    "SELECT killer, COUNT(killer) AS count FROM deaths "
                    "WHERE corpse = %s GROUP BY killer HAVING deaths.count > 0 ORDER BY count DESC LIMIT 40",
                    1,
                ),
                (corpse,),
            )
            ...
            cursor.execute(
                fmt_sql(
                    "SELECT MIN(timestamp) FROM deaths " "WHERE lower(killer) = %s;", 1
                ),
                (killer.lower(),),
            )
            try:
            ...
            cursor.execute(
                fmt_sql(
                    "SELECT corpse, COUNT(corpse) AS count FROM deaths "
...
def expunge_old_data():
    with DBContextManager() as conn:
        ...
        cursor.execute(
            "DELETE FROM deaths AS d " "WHERE d.timestamp < '%s'" % nine_days_ago
        )
...
def show_game_feed(types=("DEA", "DUE"), update=False, api_url=None):
    ...
    if not update:
        ...
        for death in feed:
            ...
            counts_for_kdr = death["type"] == "DEA" and " " not in killer
            if counts_for_kdr:
                try:
            ...
            with DBContextManager() as conn:
                setup_db_if_blank(conn)
                if get_or_create_deathsight(
                    conn,
                    external_id=str(death["id"]),
                    killer=killer,
                    corpse=corpse,
...
def recalculate_kdr():
    with DBContextManager() as conn:
        ...
        SELECT
            d.killer,
            CASE WHEN COUNT(d.corpse) = 0 THEN 1
...
def update_toon(db_connection, name, api_url=None):
    ...
    sql = "UPDATE characters SET %s WHERE name = %%s" % (
        ", ".join(("%s=%%s" % field for field in API_FIELDS)),
    )
    ...
    try:
        ...
        if arg.lower() == "hi":
            ...
            try:
                ...
                name = data.pop("name")
                print(
                    "{name} has killed {d} denizens and {a} adventurers.".format(
                        name=name,
